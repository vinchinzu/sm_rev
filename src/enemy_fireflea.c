...
enum {
  kFirefleaParam_Circle = 2,
  kFirefleaDirTableStep = 4,
  kFirefleaMaxDarkness = 12,
  kFirefleaDarknessStep = 2,
};

static const uint16 kFirefleaRadii[8] = {
...
void Fireflea_Init(void) {  // 0xA38D2D
  ...
  if (E->ffa_parameter_1 & kFirefleaParam_Circle) {
...
void Fireflea_Func_3(uint16 k) {  // 0xA38D75
  Enemy_FireFlea *E = Get_FireFlea(k);
  uint16 speed_idx = 8 * LOBYTE(E->ffa_parameter_2);
  if ((E->ffa_parameter_1 & 1) == 0)
    speed_idx += kFirefleaDirTableStep;
  E->ffa_var_02 = speed_idx;
  int v = speed_idx >> 1;
  E->ffa_var_B = kCommonEnemySpeeds_Linear[v];
  E->ffa_var_A = kCommonEnemySpeeds_Linear[v + 1];
}

void Fireflea_Func_4(uint16 k) {  // 0xA38D9C
  Enemy_FireFlea *E = Get_FireFlea(k);
  E->ffa_var_C = LOBYTE(kFirefleaRadii[HIBYTE(E->ffa_parameter_2)]);
}
...
void Fireflea_Main(void) {  // 0xA38DEE
  ...
  if (E->ffa_parameter_1 & kFirefleaParam_Circle) {
    ...
    if ((int16)(E->base.y_pos - E->ffa_var_00) < 0
      ...
      E->ffa_var_02 ^= kFirefleaDirTableStep;
    }
  }
}
...
void Fireflea_Touch(uint16 k) {  // 0xA38E6B
  ...
  NormalEnemyTouchAi();
  EnemyDeathAnimation(k, anim);
  if (sign16(fireflea_darkness_level - kFirefleaMaxDarkness))
    fireflea_darkness_level += kFirefleaDarknessStep;
}
