...
#ifdef BUNDLE_ASSETS
extern const unsigned char sm_rom_data[];
extern const size_t sm_rom_size;
#endif
...
typedef struct SnesDispatchEntry {
  void (*func)(void);
  bool is_long;
} SnesDispatchEntry;

static SnesDispatchEntry *g_dispatch_table[256];

void RegisterSnesFunc(uint32 addr, SnesFuncPtr func, bool is_long) {
  uint8 bank = addr >> 16;
  uint16 offset = addr & 0xFFFF;
  if (!g_dispatch_table[bank]) {
    g_dispatch_table[bank] = (SnesDispatchEntry *)calloc(65536, sizeof(SnesDispatchEntry));
  }
  g_dispatch_table[bank][offset].func = func;
  g_dispatch_table[bank][offset].is_long = is_long;
}

static uint16 cpu_pullWord(Cpu *cpu) {
  uint8_t low = snes_cpuRead(g_snes, ++cpu->sp);
  uint8_t high = snes_cpuRead(g_snes, ++cpu->sp);
  return low | (high << 8);
}

static uint8 cpu_pullByte(Cpu *cpu) {
  return snes_cpuRead(g_snes, ++cpu->sp);
}

bool DispatchSnesFunc(Cpu *cpu) {
  uint8 bank = cpu->k;
  uint16 offset = cpu->pc;
  if (g_dispatch_table[bank]) {
    SnesDispatchEntry *entry = &g_dispatch_table[bank][offset];
    if (entry->func) {
      entry->func();
      if (entry->is_long) {
...
// Mute filter for known acceptable divergences (per AGENTS.md verification slack)
// Returns true if diff at (frame, addr) should be muted (not printed)
static bool IsMutedDiff(int frame, size_t addr, bool is_oam) {
  // Frames ~1581-1600: OAM/animation temp RAM diffs, gameplay accurate
  if (frame >= 1581 && frame <= 1600) {
    if (is_oam) {
      // OAM addresses 0x0E-0x13, 0x100-0x101
      if ((addr >= 0x0E && addr <= 0x13) || (addr >= 0x100 && addr <= 0x101))
        return true;
    } else {
      // RAM addresses 0x38C-0x395, 0x571
      if ((addr >= 0x38C && addr <= 0x395) || addr == 0x571)
        return true;
    }
  }
  // Frame ~2792-2795: OAM/animation temp RAM diffs, gameplay accurate
  if (frame >= 2792 && frame <= 2795) {
    if (is_oam) {
      // OAM addresses 0x0E-0x13, 0x100
      if ((addr >= 0x0E && addr <= 0x13) || addr == 0x100)
        return true;
    } else {
      // RAM addresses 0x38C-0x395, 0x571
      if ((addr >= 0x38C && addr <= 0x395) || addr == 0x571)
...
uint32 PatchBugs(uint32 mode, uint32 addr) {
  ...
  // Prevent EquipmentScreenCategory_ButtonResponse from getting called when category changed
    ...
    memset(&button_config_shoot_x_saved, 0, 8); // grapple_beam_tmpD82 not cleared in BlockCollGrappleBeam
...
#ifdef BUNDLE_ASSETS
  // Use embedded ROM when bundled
  (void)name;  // name is ignored when bundled
  file = (uint8_t *)sm_rom_data;
  length = sm_rom_size;
...
void RtlRunFrameCompare(uint16 input1, uint16 input2, int run_what) {
  g_snes->input1->currentState = input1;
