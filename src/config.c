...
#ifdef BUNDLE_ASSETS
extern const unsigned char sm_config_data[];
extern const size_t sm_config_size;
#endif
...
static const uint16 kDefaultKbdControls[kKeys_Total] = {
  ...
  // VolumeUp VolumeDown SavePhysics LoadPhysics
  0, 0, _(SDLK_F11), _(SDLK_F12), _(SDLK_F10),
};
#undef _
#undef A
#undef C
...
static const KeyNameId kKeyNameId[] = {
  ...
  S(Pause), S(PauseDimmed), S(Turbo), S(ReplayTurbo), S(WindowBigger), S(WindowSmaller), S(VolumeUp), S(VolumeDown), S(DisplayPerf), S(ToggleRenderer), S(SavePhysics), S(LoadPhysics), S(MultiSamus),
};
#undef S
#undef M
...
static bool HandleIniConfig(int section, const char *key, char *value) {
  if (section == 0) {
    ...
    } else if (StringEqualsNoCase(key, "SkipIntro")) {
...
static bool ParseConfigFromBuffer(const char *buf, size_t len) {
  // Make a mutable copy since NextLineStripComments modifies the buffer
  char *filedata = (char *)malloc(len + 1);
  if (!filedata) return false;
  memcpy(filedata, buf, len);
  filedata[len] = 0;
  char *p;
  ...
  for (int lineno = 1; (p = NextLineStripComments(&filedata)) != NULL; lineno++) {
    ...
    if (*p == '[') {
      section = GetIniSection(p);
      if (section < 0)
        fprintf(stderr, "<embedded>:%d: Invalid .ini section %s\n", lineno, p);
    } else if (section == -2) {
      fprintf(stderr, "<embedded>:%d: Expecting [section]\n", lineno);
    } else {
      char *v = SplitKeyValue(p);
      if (v == NULL) {
        fprintf(stderr, "<embedded>:%d: Expecting 'key=value'\n", lineno);
        continue;
      }
      if (section >= 0 && !HandleIniConfig(section, p, v))
        fprintf(stderr, "<embedded>:%d: Can't parse '%s'\n", lineno, p);
    }
  }
  return true;
}
...
void ParseConfigFile(const char *filename) {
  ...
  (void)filename;
  ParseConfigFromBuffer((const char *)sm_config_data, sm_config_size);
#else
