...
// PhysicsParams defined in physics_config.c
...
void RtlSkipIntro(void) {
  if (!g_rtl_runframe) return;
  for (int i = 0; i < 2000 && game_state < kGameState_4_FileSelectMenus; i++) {
    g_rtl_runframe(i < 3 ? kButton_Start : 0, 0, 0);
    snes_frame_counter++;
  }
  StateRecorder_Init(&state_recorder);
}
...
bool RtlRunFrame(uint16 inputs1, uint16 inputs2) {
  // Avoid up/down and left/right from being pressed at the same time
  if ((inputs1 & 0x30) == 0x30) inputs1 ^= 0x30;
  if ((inputs1 & 0xc0) == 0xc0) inputs1 ^= 0xc0;
  if ((inputs2 & 0x30) == 0x30) inputs2 ^= 0x30;
  if ((inputs2 & 0xc0) == 0xc0) inputs2 ^= 0xc0;

  bool is_replay = state_recorder.replay_mode;

  // Either copy state or apply state
  if (is_replay) {
    inputs1 = StateRecorder_ReadNextReplayState(&state_recorder);
    inputs2 = 0;
  } else {
    // Loading a bug snapshot?
    if (state_recorder.snapshot_flags & 1) {
      state_recorder.snapshot_flags &= ~1;
      inputs1 = state_recorder.last_inputs;
      inputs2 = 0;
    } else {
    ...
    StateRecorder_Record(&state_recorder, inputs1);
  }

  if (bug_fix_counter != currently_installed_bug_fix_counter)
    RtlUpdateSnesPatchForBugfix();

  g_rtl_runframe(inputs1, inputs2, 0);
...
void RtlLoadStateFromPath(const char *path) {
