...
int main(int argc, char** argv) {
  ...
  while (argc > 0 && argv[0][0] == '-') {
    if (strcmp(argv[0], "--config") == 0 && argc >= 2) {
      config_file = argv[1];
      has_config_arg = true;
      argc -= 2, argv += 2;
      ...
      g_verbose = false;
      argc -= 1, argv += 1;
    } else if (strcmp(argv[0], "--headless") == 0 && argc >= 2) {
      ...
      argc -= 2, argv += 2;
    } else if (strcmp(argv[0], "--input-list") == 0 && argc >= 2) {
      ...
      g_input_list = (uint16 *)malloc(count * sizeof(uint16));
      g_input_list_size = count;
      free(list);
      list = strdup(argv[1]);
      token = strtok(list, ",");
      for (int i = 0; i < count; i++) {
        g_input_list[i] = (uint16)strtol(token, NULL, 16);
        token = strtok(NULL, ",");
      }
      free(list);
      argc -= 2, argv += 2;
  ...
  if (force_music && *force_music) {
    unsigned song_set = 0, track = 0;
    if (sscanf(force_music, "%u,%u", &song_set, &track) == 2) {
      QueueMusic_Delayed8(0);
      QueueMusic_Delayed8((song_set & 0xff) | 0xff00);
      QueueMusic_DelayedY(track & 0xff, 6);
    }
  }
  ...
  if (g_headless_dump) {
    ...
    if (f) {
      ...
      fprintf(f,
        ...
        "  \"area_index\": %u,\n"
        "  \"room_ptr\": %u,\n"
        "  \"roomdefroomstate_ptr\": %u,\n"
        "  \"room_music_track_index\": %u,\n"
        "  \"room_music_data_index\": %u,\n"
        "  \"room_enemy_population_ptr\": %u,\n"
        "  \"room_enemy_tilesets_ptr\": %u,\n"
        "  \"num_enemies_in_room\": %u,\n"
        "  \"spc_port0\": %u,\n"
        "  \"spc_music_ptr_toplevel\": %u,\n"
        "  \"spc_counter_sf0c\": %u,\n"
        "  \"spc_fast_forward\": %u,\n"
        "  \"spc_track3_ptr\": %u,\n"
        "  \"spc_track5_ptr\": %u,\n"
        "  \"spc_track8_ptr\": %u,\n"
        "  \"spc_chan8_ptr\": %u,\n"
        "  \"spc_chan8_start_ptr\": %u,\n"
        "  \"spc_chan8_saved_ptr\": %u,\n"
        "  \"spc_0be1\": %u,\n"
        "  \"spc_0be2\": %u,\n"
        "  \"spc_0be3\": %u,\n"
        "  \"spc_0be4\": %u,\n"
        ...
        (unsigned)(*(uint16 *)(g_ram + 0x7BB)),
        (unsigned)(*(uint16 *)(g_ram + 0x7C9)),
        (unsigned)(*(uint16 *)(g_ram + 0x7CB)),
        (unsigned)(*(uint16 *)(g_ram + 0x7CF)),
        (unsigned)(*(uint16 *)(g_ram + 0x7D1)),
        (unsigned)(*(uint16 *)(g_ram + 0xE4E)),
        (unsigned)g_spc_player->port_to_snes[0],
        (unsigned)g_spc_player->music_ptr_toplevel,
        (unsigned)g_spc_player->counter_sf0c,
        (unsigned)g_spc_player->fast_forward,
        (unsigned)(*(uint16 *)&g_spc_player->ram[0x5824]),
        (unsigned)(*(uint16 *)&g_spc_player->ram[0x5828]),
        (unsigned)(*(uint16 *)&g_spc_player->ram[0x582e]),
        (unsigned)g_spc_player->channel[7].pattern_order_ptr_for_chan,
        (unsigned)g_spc_player->channel[7].pattern_start_ptr,
        (unsigned)g_spc_player->channel[7].saved_pattern_ptr,
        (unsigned)g_spc_player->ram[0x0be1],
        (unsigned)g_spc_player->ram[0x0be2],
...
static void HandleCommand(uint32 j, bool pressed) {
  ...
  if (j <= kKeys_Load_Last) {
    ...
    case kKeys_SavePhysics: SavePhysicsConfig(); printf("Saved physics parameters to sm_physics.json\n"); break;
    case kKeys_LoadPhysics:
      LoadPhysicsConfig();
      LoadEnemyConfig();
      RtlApplyPhysicsParams();
      printf("Reloaded physics and enemy parameters.\n");
      break;
    default: assert(0);
    }
  }
