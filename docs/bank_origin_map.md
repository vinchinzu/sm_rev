...
| `src/door_transition.c` | Door-transition game states 9-11 and the Bank `$82` door pipeline | `../sm/src/sm_82.c` | `GameState_9_HitDoorBlock` `0x82E169`, `DoorTransitionFunction_SetupNewRoom` `0x82E36E`, `LoadDoorHeader` `0x82DE12`, `SpawnDoorClosingPLM` `0x82E8EB` |
| `src/room_transition.c` | Room load, library backgrounds, and StartGameplay | `../sm/src/sm_82.c` | `LoadLevelDataAndOtherThings` `0x82E7D3`, `LoadLevelScrollAndCre` `0x82EA73`, `LoadLibraryBackground`, `StartGameplay_Async` `0x80A07B` |
...
| Current file | Original bank file in `../sm/` | Notes |
| --- | --- | --- |
| `src/enemy_main.c` | `../sm/src/sm_a0.c` | Shared enemy lifecycle, spawn/load path, `EnemyMain` frame loop, and draw/runtime plumbing after the Bank `$A0` split. Unique leftover addr→fn rows live in `enemy_ai_table.c` |
| `src/enemy_ai_table.c` | `../sm/src/sm_a0.c` (`CallEnemyAi` / `CallEnemyPreInstr` / `CallEnemyInstr`) | Leftover unique addr→fn tables used at EnemyDef load, hitbox, and instruction bytecode. Drain rows as creature vtables grow. |
...
| `src/eproj_skree.c` | `../sm/src/sm_86.c` | Skree particle family (`Eproj_Init_0x8aaf` through `Eproj_PreInstr_SkreeParticle`) |
| `src/eproj_draygon.c` | `../sm/src/sm_86.c` | Draygon gunk / wall-turret family (`EprojInstr_SpawnEnemyDropsWithDraygonsEyeDrops` through `EprojPreInstr_DraygonsGunk_8E0F`) |
| `src/eproj_crocomire.c` | `../sm/src/sm_86.c` | Crocomire projectile, spike-wall, and bridge-crumbling family (`EprojInit_CrocomireProjectile` through `EprojPreInstr_CrocomireBridgeCrumbling`) |
| `src/eproj_kraid.c` | `../sm/src/sm_86.c` | Kraid rocks and Mini-Kraid spit/spikes (`EprojInit_RocksKraidSpits` through `EprojPreInstr_MiniKraidSpikes`) |
| `src/eproj_pirate.c` | `../sm/src/sm_86.c` | Pirate lasers/claws, Ceres elevator/debris, Ceres Ridley fireballs, and Shaktool attack circles |
| `src/eproj_ki_hunter.c` | `../sm/src/sm_86.c` | Ki-Hunter acid spit, Kago bugs, Maridia floater/spike-plant, Wrecked Ship robot lasers, n00b tube shards, and spore families |
| `src/eproj_botwoon.c` | `../sm/src/sm_86.c` | Botwoon body/spit plus Yapping Maw; owns `CallBotwoonEprojFunc` |
| `src/eproj_pickup.c` | `../sm/src/sm_86.c` | Pickup, death-explosion, and sparks tail; owns `RandomDropRoutine` and `RespawnEnemy` |
| `src/eproj_combat.c` | `../sm/src/sm_86.c` | Leftover Bank `$86` dispatch (`CallEprojInit` / `CallEprojPreInstr` / `CallEprojInstr`) plus unpeeled Phantoon fireballs, walking lava seahorse, remaining Bomb Torizo / Chozo / statue-dust handlers, and shared off-screen helpers `Eproj_FuncE722` / `Eproj_FuncE73E_MoveXY`. Drain: Phantoon → `eproj_phantoon.c`; seahorse → `eproj_environment.c`; Torizo/Chozo/statue-dust → `enemy_torizo_projectiles.c` / `eproj_tourian.c`; helpers → `eproj_core.c`. |
